...
import { CheckCircle, Cancel, DirectionsCar, ExpandMore, Group } from "@mui/icons-material"
...
const MOCK_ACCESSES: AccessRecord[] = [
  {
    ...
    time: "Justo ahora", status: "authorized", type: "vehicular",
    plate: "BCN-4521",
    assignedStudents: [
      { id: "s1", name: "Carlos Hernández L.", career: "Ing. Sistemas" },
      { id: "s2", name: "Mariana Hernández L.", career: "Lic. Diseño" },
      { id: "s3", name: "Daniel Hernández R.", career: "Ing. Industrial" },
    ],
  },
  ...
  {
    id: 5, name: "Ana López Castillo", role: "Alumno", gate: "Principal (A y C)",
    time: "Hace 15 min", status: "authorized", type: "vehicular",
    plate: "MXL-0012",
    assignedStudents: [
      { id: "s4", name: "Ana López Castillo", career: "Lic. Mercadotecnia" },
      { id: "s5", name: "Sofía López Castillo", career: "Ing. Civil" },
    ],
  },
]
...
export function GuardRealtime() {
  ...
  // Reusable expandable students panel
  const StudentsPanel = ({ access }: { access: AccessRecord }) => {
    const isExpanded = expandedId === access.id
    const count = access.assignedStudents?.length || 0
    if (access.type !== 'vehicular' || !access.plate || count === 0) return null

    return (
      <div className="border-t border-[#E2E8F0]">
        <button
          onClick={(e) => { e.stopPropagation(); setExpandedId(isExpanded ? null : access.id) }}
          className="w-full px-4 py-2.5 flex items-center justify-between gap-3 hover:bg-[#F8FAFC] transition-colors"
        >
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-7 h-7 rounded-md bg-blue-50 flex items-center justify-center shrink-0">
              <DirectionsCar fontSize="small" className="text-[#2563EB]" style={{ fontSize: 16 }} />
            </div>
            <span className="font-mono font-bold text-[12px] text-[#0A1628]">{access.plate}</span>
            <span className="flex items-center gap-1 text-[11px] font-semibold text-[#64748B] bg-[#F1F5F9] px-2 py-0.5 rounded-full">
              <Group style={{ fontSize: 12 }} />{count} alumnos
            </span>
          </div>
          <ExpandMore
            fontSize="small"
            ...
            >
              <div className="px-4 py-3 space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#64748B] mb-2">
                  Alumnos asignados al vehículo
                </p>
                {access.assignedStudents!.map(s => (
                  <div key={s.id} className="flex items-center gap-3 bg-white rounded-lg border border-[#E2E8F0] px-3 py-2">
                    <div className="w-8 h-8 rounded-full bg-[#2563EB] text-white flex items-center justify-center font-bold text-[11px] shrink-0">
                      {initials(s.name)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-semibold text-[#0A1628] truncate">{s.name}</p>
                      {s.career && <p className="text-[11px] text-[#64748B] truncate">{s.career}</p>}
                    </div>
                    {s.name === access.name && (
                      <span className="text-[9px] font-bold uppercase tracking-wider text-[#16A34A] bg-[#16A34A]/10 px-1.5 py-0.5 rounded">
                        En acceso
                      </span>
                    )}
                  </div>
      ...
      <div className="space-y-4">
        <AnimatePresence>
          {filteredAccesses.map((access, i) => {
            ...
            // Compact cards (with optional expandable panel below)
            return (
              <motion.div
                key={access.id}
                ...
                transition={{ delay: i * 0.1, duration: 0.3 }}
                className={`bg-white rounded-[12px] shadow-[0px_1px_3px_rgba(0,0,0,0.05)] border overflow-hidden flex flex-col ${
